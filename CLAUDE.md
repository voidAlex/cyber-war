...
- **后端（爪牙）**：Rust（Tauri 2）+ reqwest（LLM 流式转发）+ aes-gcm + pbkdf2（加密）
...
| `cargo test` | Rust 单测（fs 原子写/追加/crypto） |
| `pnpm lint` | ESLint 全量检查 |

## 架构铁律

- **前端 = 大脑**：TDD 六层分层，承载状态机、领域结算、Agent 编排、物理 Worker、持久化编排。
- **Rust = 爪牙**：只做三件事——本地文件 IO / LLM 真流式转发 / 加密原语。**Rust 零业务逻辑**。
...
```
src-tauri/        Rust 后端：fs/(原子写/真追加) llm/(router/stream/guard) crypto/(kdf/aead)
...
- 代码加**方法级和文件级中文注释**。
- 遵守 **Rust 零业务逻辑**铁律；命令接口仅 fs/crypto/llm 三类。
- 每次对话结尾加 **"喵~"**。
