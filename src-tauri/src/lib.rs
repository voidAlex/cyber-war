...
///   与 logs 根目录（`<app_data_dir>/logs`，全局应用日志 app.log 落盘前置）。
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // 初始化 logs 根目录（不存在则创建）
            // 全局应用日志 app.log 的落盘前置；fs_append_app_log 命令内部也会 ensure 兜底。
            let logs_root = fs::resolve_logs_root(app.handle())?;
            if !Path::new(&logs_root).exists() {
                std::fs::create_dir_all(&logs_root).map_err(|e| {
                    Box::new(std::io::Error::new(
                        std::io::ErrorKind::Other,
                        format!("创建 logs 根目录失败: {e}"),
                    )) as Box<dyn std::error::Error>
                })?;
            }
            Ok(())
